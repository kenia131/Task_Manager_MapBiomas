...
class TemporaryCropsSettings(DefaultSettings):
    ...
    # GRID_FILTER = And(Is('AC_WRS'), Or(Is('CERRADO'), Is('AMAZONIA'), Is('SC'), Is('PR'), Is('RS')))

    # < 2020
    GRID_FILTER = And(Is('AC_WRS'),
        Or(
            And(Is('CERRADO'), Or(Is('CAATINGA'), Is('MG'), Is('BA'))),
            Is('SP')
        )
    )

    # AC_WRS && ((CERRADO && (CAATINGA || MG || BA )) || SP)
    ...
    EXPORT_DIRECTORY = 'mapbiomas_c6/classification/temporary_crops/vC5/cerrado_amazonia_sul/{year}'
    # EXPORT_DIRECTORY = 'mapbiomas_c6/classification/temporary_crops/vC5/{year}-2'
...
SettingsManager.add_settings(TemporaryCropsSettings)
